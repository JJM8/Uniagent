...
def model_switch(c):
    ...
    Read BEFORE continue_from(), which takes the marker carrying the answer off
    ...
    changes nothing about which model the retry goes to - it is what lets the
    retry SAY so. A request that failed on an overloaded provider, a model
    picked in the corner, and then a continue, is the whole reason the button
    exists; without this the turn quietly goes somewhere else than the one that
    failed, and nothing on the page or in the terminal accounts for it.
    ...
    # pin the corner just wrote is the whole point of asking.
    c.reload_model()
    prov, mod, _ = c.models()
    if (prov, mod) == was:
        return None
...
def switch_note(switch):
    """model_switch()'s answer as one line for a person to read, or "" for
    None - so the page, the terminal and anything later say the same thing."""
    if not switch:
        return ""
