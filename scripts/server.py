...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if text.strip().lower() == main.CONTINUE:
            ...
            # pair either way - main.turn re-reads it as it starts - so this is
            # only so that a continue landing on a different provider than the
            # one that just failed says so, instead of being a silent switch.
