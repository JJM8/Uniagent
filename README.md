...
Once it fires, the session stays open for a while (`wake_session_ms`), so you can carry on talking, or answer a question, without saying the phrase again. Only then does a sentence get cut out on silence and sent to the same `POST /voice` the hold-to-talk button uses, on the same provider.

**Being slow to talk is handled rather than punished.** Nothing can tell a pause in the middle of a sentence from the end of one, so this doesn't pretend to: it waits a beat, sends, and corrects itself if you turn out not to have finished. Carry on talking and the turn that just started is stopped, and everything you said goes again as one message with the late half marked `voice_continued:` -- so the model answers the sentence you actually said instead of the first half of it. The wait before sending is affordable because nothing idles through it: each clip is transcribed *while* it runs, so raising `wake_submit_ms` costs the silence and not a second of transcription on top.

The interrupt has one hard limit. A turn that has already called a tool is never unwound -- it ran something, and a history rewound past that would have the model do it twice -- so that turn keeps everything it did and the late words go on top of it as their own message. The page says so when it happens, because the transcript then shows a stop nobody pressed. Turning the interrupt off instead folds late words into the running turn at its next tool result, exactly as pressing enter mid-turn does.
