...
**Both ways of speaking to it are one feature.** The button and the wake word below share the same capture, the same live caption and the same way out: the finished text goes through `sendUserText()` and `POST /input`, exactly as a typed message does, so it queues behind a running turn and folds into one mid-turn in exactly the same way. The only difference between them is what opens and closes the session -- a finger on a button, or a phrase and a silence. They used to be two code paths, the wake word posting to a route of its own, and that is precisely how one sentence could end up in two different chats.
...
**Only one window listens at a time.** The wake word is the one part of this that runs without anybody looking at it, so every open tab used to start its own listener -- and a sentence spoken near two of them was detected twice, transcribed twice, and sent into whichever chat each tab happened to be sitting in. Tabs of one browser now settle it between themselves with a lease in `localStorage`, heartbeated so a tab that dies without releasing it goes stale rather than locking the feature out; a tab refused the lease says so instead of silently doing nothing, and takes over when the listening one closes. Two separate *devices* are two browsers and cannot share a lease, so the server has the last word there: an identical spoken message arriving twice within five seconds is taken once. Only spoken messages are checked -- typing the same thing twice on purpose is a real thing people do.
...
There's also an optional `voice_captions` setting, off by default, that re-transcribes the whole clip so far roughly once a second while you talk so a caption can follow along above the message box -- whatever it says REPLACES what's shown, not an addition to it. It covers **both** ways of speaking, since both record the same way. It's a nicety, not a requirement: it isn't what decides you've stopped talking, and because each poll re-sends everything said so far rather than just what's new, it multiplies what a session bills -- ten seconds of talking bills roughly what fifty-five would normally cost. Leave it off unless you want the live feedback badly enough to pay for it.
...
### Uncached-token warnings

The token bar answers "how full is the context window". Under it is the other question, the one that shows up on the bill: **how much of the next request the provider will have to read at full price.**

Prompt caching is a byte-for-byte match on the *front* of the prompt. A provider keeps a prompt it has already processed and charges about a tenth of the price for however much of the next one matches it from the beginning -- which means a single changed byte anywhere invalidates everything after it. So the only question worth asking is where this request and the last one stop being the same, and that is what the panel now says: `47,200 uncached of 51,000 - the tool list changed`.

It is a prediction, made *before* the tokens are spent. After every request Uniagent writes down a fingerprint of what it actually sent -- one short digest per piece, in the order the endpoint reads them: the tool schemas, the system prompt, then each turn -- into `cache.json` beside the chat's transcript. Before the next one it renders what *would* go, diffs the two, and counts the tokens from the first difference onward. Digests rather than text, because the only operation ever performed on them is equality and keeping the text would mean a second copy of every chat on disk.

Naming the cause is most of the value, because three of the four are mistakes that are cheap to fix while the request is still hypothetical and expensive to notice on a bill:

- **the tool list changed** -- the agent wrote itself a tool mid-conversation, and tool schemas render at byte zero, so the whole prefix goes
- **the system prompt changed** -- a `context/` file or a memory was edited
- **the history was rewritten** -- a turn already sent went back and changed; the expensive one, and the easiest to do by accident
- **the cache has probably expired** -- nothing is wrong, it has simply been a while. Recomputed in the browser on its own clock, since a cache expiring is the one cause that arrives without anything changing for the server to notice.

**Every prediction is settled against what actually happened.** The provider's own `cache_read` figure is filed against the request that was predicted about, and shown next to it. A "should hit cache" that comes back having read zero is not a rounding error -- it is a silent invalidator nobody has found yet, and there is no way to see one without both numbers side by side. `/cache` says the same thing as text.

**Who caches, and who has to be asked.** Most endpoints cache by themselves and simply report what they hit; Anthropic caches nothing unless the request marks where the reusable part ends. Uniagent sends those marks -- `cache_control` breakpoints on the system prompt and on the newest turn, two of the four allowed -- for Anthropic directly and for Anthropic models routed through OpenRouter, which passes them along. Which one an endpoint is, is decided by the **host** it points at rather than by its wire: an OpenRouter card and a local llama.cpp are both normally plain "openai" wires, and one of them caches. Anything not recognised is reported as having no cache rather than being given a plausible number -- a wrong reassurance is worse than silence, because it is a figure somebody would plan around.
...
**One picker, everywhere.** The box beside the message input and every model field on the settings page -- main agent, subagents, cron jobs, the safety tab's checking model, the voice tab's summariser -- open the same panel: type to search across every provider and model at once, arrow keys to move, enter to take it. The five models you picked most recently sit at the top of it, and they are the same five on every device and in the terminal, because they are recorded on the server rather than in a browser. A search that matches nothing offers to use what you typed anyway, on whichever provider -- a model id newer than any catalogue has always worked here and still does.

**Which company actually serves it.** A router like OpenRouter forwards one model id to any of a dozen companies, and they are not interchangeable: different quantisations, context limits, prices and outages. Pick a model on a routed provider and the panel grows a box for the endpoint -- typed in as the router's own slug (`baidu/fp8`), or clicked from the list it fetches, with quantisation, context, price and uptime on each. Empty means what it always meant: let the router choose. The choice belongs to the provider/model pair, so it holds in this chat, in a subagent and in a cron job at 4am, and it is recorded next to that model's other settings in `models_custom.json`. Under the hood it is one more thing a wire can describe (see `routes` in `defaults/wires.json`); a provider pointed at `openrouter.ai` gets it whichever wire it is on.
